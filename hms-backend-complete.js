        ...
        if (userResult.rows.length === 0) {
            ...
            const newUser = await pool.query(
                `INSERT INTO users (
                    ...
                    "isEmailVerified", "isPhoneVerified", "loginAttempts", "created_at", "updated_at"
                ) 
                VALUES (
...
app.get('/api/medical-records', authenticateToken, async (req, res) => {
    try {
        ...
        let query = `
            ...
            LEFT JOIN staff sm ON mr.doctor_id = sm.id
        `;
        
        if (patient_id) {
            query += ` WHERE mr.patient_id = ${patient_id}`;
        }
...
app.post('/api/medical-records', authenticateToken, async (req, res) => {
    try {
        const { patient_id, diagnosis, symptoms, treatment, doctor_id, visit_type } = req.body;
        
        const result = await pool.query(
            `INSERT INTO medical_records (patient_id, diagnosis, symptoms, treatment, doctor_id, visit_type, created_at, updated_at) 
             VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW()) 
             RETURNING *`,
            [patient_id, diagnosis, symptoms, treatment, doctor_id || req.user.id, visit_type]
        );
...
app.get('/api/billing/bills', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(
...
app.post('/api/billing/bills', authenticateToken, async (req, res) => {
    try {
        const { patient_id, items, total_amount, due_date, insuranceProvider, insuranceCoverage } = req.body;
        
        const bill_number = 'INV-' + Date.now();
        const netAmount = total_amount - (insuranceCoverage || 0);
        
        const result = await pool.query(
            `INSERT INTO bills (bill_number, patient_id, total_amount, due_date, status, created_at, updated_at) 
             VALUES ($1, $2, $3, $4, 'pending', NOW(), NOW()) 
             RETURNING *`,
            [bill_number, patient_id, total_amount, due_date]
        );
...
app.post('/api/inventory/add-stock', authenticateToken, async (req, res) => {
    try {
        const { name, category, quantity, unit, reorder_level, expiry_date, supplier } = req.body;
        ...
        if (existing.rows.length > 0) {
            ...
            result = await pool.query(
                `UPDATE inventory_items 
                 SET quantity = quantity + $1, reorder_level = $2, expiry_date = $3, updated_at = NOW() 
                 WHERE id = $4 
                 RETURNING *`,
                [quantity, reorder_level, expiry_date, existing.rows[0].id]
            );
        } else {
            // Create new item
            result = await pool.query(
                `INSERT INTO inventory_items (item_name, category, quantity, unit, reorder_level, expiry_date, supplier, created_at, updated_at) 
                 ...
                 RETURNING *`,
                [name, category, quantity, unit, reorder_level, expiry_date, supplier]
            );
        }
        ...
        if (result.rows[0].quantity <= result.rows[0].reorderlevel) {
            broadcast({ 
                ...
                data: { 
                    ...
                    reorder_level: result.rows[0].reorderlevel 
                } 
            });
        }
...
app.get('/api/staff/roster', authenticateToken, async (req, res) => {
    try {
        ...
        let query = `
            ...
            JOIN staff s ON ws.staff_id = s.id::text 
            LEFT JOIN Department d ON s.departmentId = d.id
        `;
...
app.post('/api/beds/admission', authenticateToken, async (req, res) => {
    try {
        const { patient_id, departmentId, bedNumber, admissionReason, doctor_id } = req.body;
        
        // Update patient status
        await pool.query(
            `UPDATE patients SET status = 'admitted', departmentid = $1, bednumber = $2, updated_at = NOW() 
             WHERE id = $3`,
            [departmentId, bedNumber, patient_id]
        );
        ...
        const result = await pool.query(
            `INSERT INTO encounters (patient_id, type, admission_date, department_id, bed_number, reason, attending_doctor_id, status) 
             ...
             RETURNING *`,
            [patient_id, departmentId, bedNumber, admissionReason, doctor_id]
        );
...
app.post('/api/beds/discharge', authenticateToken, async (req, res) => {
    try {
        const { patient_id, dischargeNotes } = req.body;
        
        // Update patient status
        await pool.query(
            `UPDATE patients SET status = 'discharged', departmentid = NULL, bednumber = NULL, updated_at = NOW() 
             WHERE id = $1`,
            [patient_id]
        );
...
app.get('/api/analytics/dashboard', authenticateToken, async (req, res) => {
    try {
        ...
        const admissions = await pool.query("SELECT COUNT(*) as total FROM patients WHERE status = 'admitted'");
        const revenue = await pool.query("SELECT COALESCE(SUM(total_amount), 0) as total FROM bills WHERE status = 'paid'");
        ...
        switch (reportType) {
            case 'patients':
                const patients = await pool.query(
                    `SELECT * FROM patients WHERE created_at BETWEEN $1 AND $2`,
                    [startDate, endDate]
                );
                data = patients.rows;
                break;
            case 'revenue':
                const revenue = await pool.query(
                    `SELECT * FROM bills WHERE created_at BETWEEN $1 AND $2`,
                    [startDate, endDate]
                );
                data = revenue.rows;
                break;
            case 'inventory':
                const inventory = await pool.query('SELECT * FROM inventory_items');
                data = inventory.rows;
                break;
            default:
                data = [];
...
app.post('/api/prescriptions', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `INSERT INTO Prescription (patient_id, doctor_id, status, created_at, updated_at) 
    ...
    try {
        const result = await pool.query(
            `SELECT a.*, p.name as patient_name, sm.name as doctor_name 
             ...
             LEFT JOIN staff sm ON a.doctor_id = sm.id 
...
app.post('/api/appointments', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `INSERT INTO Appointment (patient_id, doctor_id, date, time, reason, status, created_at, updated_at) 
...
app.post('/api/lab-results', authenticateToken, async (req, res) => {
    try {
        const { patient_id, testType, results, doctor_id } = req.body;
        
        const result = await pool.query(
            `INSERT INTO LabResult (patient_id, testType, results, orderedBy, status, created_at, updated_at) 
             VALUES ($1, $2, $3, $4, 'completed', NOW(), NOW()) 
             RETURNING *`,
            [patient_id, testType, JSON.stringify(results), doctor_id || req.user.id]
        );
